...
function [X, errList] = HaLRTC(T, Omega, alpha, beta, maxIter, epsilon, T_org)
...
for k = 1: maxIter
    if mod(k, 20) == 0
        fprintf('HaLRTC: iterations = %d   difference=%f  NMAE=%f  \n', k, errList(k-1), calc_NMAE(T_org, Omega, X));
    end
    beta = beta * 1.05;
